    ...
    pub async fn fetch_web_content(&self, url: &str) -> Result<String, RssError> {
        ...
        for attempt in 0..max_retries {
            match self.fetch_web_content_once(url).await {
                ...
                Err(e) => {
                    last_error = Some(e);
                    log::warn!("Attempt {}/{} failed for {}: {:?}", attempt + 1, max_retries, url, last_error);
        ...
        for attempt in 0..max_retries {
            match self.fetch_feed_once(url).await {
                Ok(articles) => {
                    ...
                    log::info!("Successfully fetched {} articles from {}", articles.len(), url);
                    return Ok(articles);
                },
                Err(e) => {
                    last_error = Some(e);
...
async fn fetch_feed_once(&self, url: &str) -> Result<Vec<Article>, RssError> {
        ...
        let content_type_str = response.headers().get("content-type")
        ...
        log::debug!("First {} bytes of response: {:?}", preview_size, &body[..preview_size]);
        ...
        if let Ok(text) = String::from_utf8(body.to_vec()) {
            ...
            if lower_text.contains("<!doctype html") || lower_text.contains("<html") {
                ...
                if lower_text.contains("404") || lower_text.contains("not found") {
                    return Err(RssError::NetworkError("服务器返回404错误，该RSS源可能不存在或已更改".to_string()));
                }
                ...
                if let Some(rss_link) = self.extract_rss_link_from_html(&text) {
                    ...
                    return Err(RssError::NetworkError(format!("检测到HTML页面，已提取RSS链接: {}", rss_link)));
                }
            }
        }
        ...
        // 如果所有尝试都失败，则返回错误
        Err(RssError::NetworkError("无法解析RSS/Atom内容，尝试了多种编码和解析方法".to_string()))
    }
    ...
    #[allow(unused)]
    pub async fn fetch_multiple_feeds(&self, urls: &[&str]) -> HashMap<String, Result<Vec<Article>, RssError>> {
        ...
        for &url in urls {
            ...
            let task = tokio::spawn(async move {
                (url_str, fetcher.fetch_feed(&url_clone).await)
            });
...
for task in tasks {
    ...
    fn item_to_article(&self, item: &Item, source: &str) -> Article {
       let pub_date = if let Some(date_str) = item.pub_date() {
        ...
        Article {
            ...
            guid: item.guid().map_or_else(|| item.link().unwrap_or("").to_string(), |guid| guid.value().to_string()),
        }
    }
    ...
    fn parse_pub_date(&self, date_str: &str) -> Result<DateTime<Utc>, chrono::ParseError> {
        ...
        if trimmed_date_str.contains(",") && trimmed_date_str.contains("GMT") {
            ...
            if let Ok(date) = DateTime::parse_from_str(&modified_date_str, "%a, %d %b %Y %H:%M:%S %z") {
                return Ok(date.with_timezone(&Utc));
            }
        }
        ...
        DateTime::parse_from_str("invalid-date", "%Y-%m-%d")
    ...
    fn sanitize_content(&self, html: &str) -> String {
        // 替换脚本标签
        let mut clean_html = html.replace("<script", "<noscript")
                               .replace("</script>", "</noscript>");
        
        // 移除事件处理器和JavaScript
        clean_html = clean_html.replace(" on", " data-on")
                             .replace("javascript:", "")
                             .replace("javascript ", "");
        // 移除VBScript
    ...
    fn try_decompress_gzip(&self, bytes: &[u8]) -> Result<Vec<u8>, std::io::Error> {
        ...
        if bytes.len() >= 2 && bytes[0] == 0x1F && bytes[1] == 0x8B {
            ...
            match decoder.read_to_end(&mut decompressed) {
                Ok(_) => {
                    log::debug!("Successfully decompressed with gzip decoder, size: {} bytes", decompressed.len());
        ...
        match decoder.read_to_end(&mut decompressed) {
            Ok(_) => {
                log::debug!("Successfully decompressed with MultiGzDecoder, size: {} bytes", decompressed.len());
                return Ok(decompressed);
            },
            Err(e) => {
        ...
        match decoder.read_to_end(&mut decompressed) {
            Ok(_) => {
                log::debug!("Successfully decompressed with DeflateDecoder, size: {} bytes", decompressed.len());
                return Ok(decompressed);
            },
            Err(e) => {
    ...
    fn extract_rss_link_from_html(&self, html: &str) -> Option<String> {
        ...
        if let Ok(pattern) = regex::Regex::new(r#"<link[^>]+rel=['"]alternate['"].*?type=['"]application/rss\+xml['"].*?href=['"]([^'"]+)['"]"#) {
        ...
        if let Ok(pattern) = regex::Regex::new(r#"<link[^>]+rel=['"]alternate['"].*?type=['"]application/atom\+xml['"].*?href=['"]([^'"]+)['"]"#) {
            if let Some(caps) = pattern.captures(html) {
                return Some(caps[1].to_string());
            }
        }
    ...
    fn try_decode_utf16(&self, bytes: &[u8]) -> Vec<u16> {
        ...
        // 跳过BOM
        let start_idx = if bytes.len() >= 2 && (bytes[0] == 0xFE && bytes[1] == 0xFF || bytes[0] == 0xFF && bytes[1] == 0xFE) {
            2
        } else {
            0
        };
    ...
    /// 从OPML字符串导入订阅源
    #[allow(unused)]
    pub async fn import_from_string(&self, opml_content: &str) -> Result<Vec<(String, String, String)>, anyhow::Error> {
        let document = roxmltree::Document::parse(opml_content)?;
        
        let mut feeds = Vec::new();
        
        // 寻找所有outline元素
        for outline in document.descendants().filter(|n| n.tag_name().name() == "outline") {
            if let Some(type_attr) = outline.attribute("type") {
                if type_attr == "rss" || type_attr == "atom" {
                    if let (Some(title), Some(url)) = (outline.attribute("title").or(outline.attribute("text")), outline.attribute("xmlUrl")) {
                        // 获取层级分组名称
                        let group = if let Some(parent) = outline.parent() {
                            if parent.tag_name().name() == "outline" {
                                parent.attribute("title").or(parent.attribute("text")).unwrap_or("").to_string()
                            } else {
                                String::new()
                            }
                        } else {
    ...
    pub async fn generate_opml(&self, feeds: &[(String, String, String)]) -> String {
        ...
        for (title, url, group) in feeds {
            groups.entry(group.clone()).or_insert_with(Vec::new).push((title, url));
        }
        
        if let Some(no_group_feeds) = groups.get(&""[..]) {
            for (title, url) in no_group_feeds {
                opml.push_str(&format!("<outline type=\"rss\" title=\"{}\" text=\"{}\" xmlUrl=\"{}\"/>", 
                    html_escape::encode_text(title), html_escape::encode_text(title), html_escape::encode_text(url)));
            }
        }
        ...
        for (group_name, feeds) in groups {
            if !group_name.is_empty() {
                opml.push_str(&format!("<outline text=\"{}\" title=\"{}\">\n", 
                    html_escape::encode_text(&group_name), html_escape::encode_text(&group_name)));
    ...
    /// 从OPML文件导入订阅源
    #[allow(unused)]
    pub async fn import_from_file<P: AsRef<std::path::Path>>(&self, path: P) -> Result<Vec<(String, String, String)>, anyhow::Error> {
    ...
    fn parse_atom_content(&self, content: &str, feed_url: &str) -> Result<Vec<Article>, RssError> {
        ...
        // 查找所有文章条目
        let entries: Vec<_> = root.descendants()
        ...
        for entry in entries {
            ...
            let pub_date = entry.descendants()
                .find(|n| n.is_element() && n.tag_name().name() == "published")
                .or_else(|| entry.descendants().find(|n| n.is_element() && n.tag_name().name() == "updated"))
                .and_then(|n| n.text())
                .and_then(|text| {
            ...
            // 提取内容
            let content_str = entry.descendants()
            ...
            // 提取作者
            let author = entry.descendants()
                .find(|n| n.is_element() && n.tag_name().name() == "author")
                .and_then(|author_elem| author_elem.descendants()
...
impl OpmlHandler {
    ...
    /// 验证订阅源是否有效
    #[allow(unused)]
    pub async fn validate_feeds(&self, feeds: &[(String, String, String)]) -> Result<Vec<(String, String, String, bool)>, anyhow::Error> {
    ...
    pub async fn export_to_file<P: AsRef<std::path::Path>>(
        ...
        if let Some(no_group_feeds) = groups.get(&""[..]) {
            for (title, url) in no_group_feeds {
                opml.push_str(&format!("<outline type=\"rss\" title=\"{}\" xmlUrl=\"{}\"/>", 
                    html_escape::encode_text(title), html_escape::encode_text(url)));
            }
        }
