...
impl PerFeedIndex {
    ...
    fn count_docs_with_term(&self, term: &str) -> usize {
        self.inverted_index.get(term).map_or(1, |ids| ids.len()).max(1)
    }
    ...
    fn remove_article(&mut self, article_id: i64) {
        ...
        // 清理空的条目以节省内存
        self.inverted_index.retain(|_term, article_ids| !article_ids.is_empty());
    }
    ...
    /// 计算文章的相关度分数
    fn calculate_relevance_score(&self, article: &Article, terms: &[String], query: &str, exact_matches: &HashSet<i64>) -> f64 {
        let mut score = 0.0;
        let article_id = article.id;
    ...
    fn search(&self, query: &str) -> Vec<Article> {
        ...
        let mut scored_articles: Vec<(Article, f64)> = partial_matches
            ...
            .filter_map(|id| {
                if let Some(article) = self.articles.get(id) {
                    let score = self.calculate_relevance_score(article, &terms, &query_lower, &exact_matches);
        ...
        // 只返回文章，不返回分数
        scored_articles.into_iter().map(|(article, _score)| article).collect()
    }
}
...
impl ArticleIndex {
    ...
    pub fn add_article(&mut self, article: Article) {
        ...
        // 获取或创建该订阅源的索引
        let feed_index = self.feed_indices.entry(feed_id).or_insert_with(PerFeedIndex::new);
    ...
    pub fn len(&self) -> usize {
        self.feed_indices.values().map(|index| index.articles.len()).sum()
    }
    
    /// 检查索引是否为空
    #[allow(unused)]
    pub fn is_empty(&self) -> bool {
        self.feed_indices.is_empty() || self.feed_indices.values().all(|index| index.articles.is_empty())
    }
}
...
impl SearchManager {
    ...
    async fn cache_results(&self, cache_key: &str, results: Vec<Article>) {
        if let Ok(mut cache) = self.search_cache.write() {
            cache.insert(cache_key.to_string(), SearchCacheEntry {
                results,
