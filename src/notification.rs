...
impl NotificationManager {
    ...
    pub fn set_ui_sender(&mut self, ui_sender: Sender<UiMessage>) {
        ...
        // 更新通知处理线程的UI发送器
        if let Err(e) = self.event_sender.send(NotificationEvent::UpdateUiSender(ui_sender)) {
            error!("更新UI发送器失败: {:?}", e);
        }
    }
    
    /// 通知处理线程
    fn notification_thread(receiver: Receiver<NotificationEvent>, config: Arc<Mutex<NotificationConfig>>, mut ui_sender: Option<Sender<UiMessage>>) {
        // 通知队列，用于控制短时间内的通知数量
            ...
            match receiver.recv_timeout(QUEUE_CLEAN_INTERVAL) {
                Ok(event) => match event {
                    NotificationEvent::NewArticles(feed_articles) => {
                        ...
                        // 限制通知数量
                        let articles_to_notify = &feed_articles[0..std::cmp::min(feed_articles.len(), config.max_notifications)];
                        
                        // 发送通知
                        for (feed_title, article) in articles_to_notify {
                            Self::send_notification(
                                &format!("[{feed_title}] {}", Self::truncate_text(&article.title, 50)),
                    ...
                    NotificationEvent::UpdateConfig(enabled, max_notifications, timeout_ms) => {
                        ...
                        info!("通知配置已更新: enabled={}, max_notifications={}, timeout_ms={}", 
    ...
    /// 发送单个通知
    fn send_notification(summary: &str, body: &str, timeout_ms: u64, article: Article, _ui_sender: Option<Sender<UiMessage>>) {
        // 使用notify-rust发送通知
        ...
        match result {
            Ok(_notification_handle) => {
                ...
                info!("Windows通知已使用应用ID 'RustRSSReader.UniqueAppID' 发送，请在系统设置中查找此应用名称");
                ...
                std::thread::spawn(move || {
                    ...
                    #[cfg(not(target_os = "windows"))]
                    {
                        ...
                        if let Ok(handle) = <_ as std::convert::TryInto<notify_rust::NotificationHandle>>::try_into(_notification_handle) {
                                        ...
                                        if let Some(id_str) = action.strip_prefix("view_") {
                                            if let Ok(id) = id_str.parse::<i64>() {
                                                ...
                                                if let Some(sender) = ui_sender_clone.as_ref() {
                                                    if let Err(e) = sender.send(UiMessage::SelectArticle(id)) {
                                                        error!("发送SelectArticle消息失败: {:?}", e);
                                                    } else {
                                                        info!("已发送SelectArticle消息，文章ID: {}", id);
                                                    }
                                                }
                                            }
                                        }
            ...
            Err(e) => {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    error!("注意：Windows可能将通知显示为关联到PowerShell，这是控制台应用的常见行为");
                    error!("建议：考虑将应用打包为Windows应用程序，或使用Windows特定的通知API以获得更好的识别");
                }
            }
        }
    }
    ...
    pub fn notify_new_articles(&self, feed_articles: Vec<(String, Article)>) {
        ...
        if let Err(e) = self.event_sender.send(NotificationEvent::NewArticles(feed_articles)) {
            error!("发送通知事件失败: {:?}", e);
        }
    }
