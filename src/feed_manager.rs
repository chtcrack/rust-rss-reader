...
use crate::utils::{is_valid_url, get_domain_from_url};
use std::sync::Arc;
...
impl FeedManager {
    ...
    pub async fn rename_group(&self, old_name: &str, new_name: &str) -> anyhow::Result<()> {
        ...
        storage.update_group_name(old_name, new_name_trimmed).await?;
    ...
    /// 统计每个分组的订阅源数量
    #[allow(unused)]
    pub async fn get_group_stats(&self) -> anyhow::Result<std::collections::HashMap<String, usize>> {
        let storage = self.storage.lock().await;
        let feeds = storage.get_all_feeds().await?;
