...
fn test_auto_update() -> Result<(), eframe::Error> {
    ...
    runtime.block_on(async {
        ...
        // 初始化RSS获取器
        println!("初始化RSS获取器...");
        let rss_fetcher = Arc::new(Mutex::new(RssFetcher::new("Rust RSS Reader Test/1.0".to_string())));
        
        // 测试1: 执行自动更新
        println!("\n测试1: 执行自动更新...");
        // 初始化搜索管理器
        let search_manager = Some(Arc::new(Mutex::new(SearchManager::new())));
        match perform_auto_update(storage.clone(), rss_fetcher.clone(), None, ui_tx.clone(), search_manager.clone()).await {
