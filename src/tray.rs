...
fn create_tray_item(config: TrayIconConfig) -> Result<TrayItemLib, Box<dyn std::error::Error>> {
    ...
    log::info!("[DEBUG] Creating tray with unique title: {}, config: {:?}", title, config);
    
    // 尝试多种图标源策略
    // 1. 首先尝试使用Windows API直接加载默认图标，然后通过RawIcon变体传递
    #[cfg(target_os = "windows")]
    {{
        use windows_sys::Win32::{Foundation::HMODULE, UI::WindowsAndMessaging::{LoadIconW, IDI_APPLICATION}};
        ...
        unsafe {
            ...
            if hicon != 0 {
                match TrayItemLib::new(&title, tray_item::IconSource::RawIcon(hicon)) {
                    Ok(tray) => {
                        log::info!("[DEBUG] Successfully created tray icon with RawIcon (Windows default icon)");
                        return Ok(tray);
                    },
                    Err(e) => {
                        log::warn!("[DEBUG] Failed to create tray with RawIcon: {}, error type: {:?}", e, e);
                    }
                }
            } else {
    ...
    match TrayItemLib::new(&title, tray_item::IconSource::Resource("IDI_APPLICATION")) {
        ...
        Err(e) => {
            log::warn!("[DEBUG] Failed to create tray with resource name IDI_APPLICATION: {}, error type: {:?}", e, e);
            ...
            match TrayItemLib::new(&title, tray_item::IconSource::Resource("")) {
                ...
                Err(e) => {
                    log::warn!("[DEBUG] Failed to create tray with empty resource string: {}, error type: {:?}", e, e);
                    Err(Box::new(e))
                }
            }
        }
...
fn run_tray_thread(
    ...
    for config in [
        ...
        match create_tray_item(config.clone()) {
            ...
            Err(e) => {
                log::warn!("Failed to create tray with config: {:?}, error: {}", config, e);
                // 递增等待时间，避免频繁重试
